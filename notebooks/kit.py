...
def cvar_historic(r, level=5):
    ...
    elif isinstance(r, pd.DataFrame): return r.aggregate(cvar_historic, level=level)
...
def plot_ef2(n_points, expected_returns, cov, style = '.'):
    ...
    """
    if expected_returns.shape[0] != 2 or expected_returns.shape[0] != 2:
...
def msr(riskfree_rate, expected_returns, cov):
    """
...
def plot_ef(n_points = 100, expected_returns = None, cov = None, style='-', legend= False, show_cml= False, riskfree_rate= 0, show_ew= False, show_gmv= False):
    """
    ...
    ax = ef.plot.line(x="Volatility", y="Returns", style=style, legend=legend, color= 'orange')
    ...
    if show_cml:
        ...
        ax.plot(cml_x, cml_y, color='green', marker='x', linestyle='dashed', linewidth=2, markersize= 7.5)
        ax.text( vol_msr, r_msr + 0.03, 'MSR', color='g' )
    ...
    if show_ew:
        ...
        # EW
        ax.plot([vol_ew], [r_ew], color='red', marker='o', markersize= 5)
        ax.text( vol_ew + 0.005, r_ew + 0.005, 'EW', color='r' )
    ...
    if show_gmv:
        ...
        # GMW
        ax.plot([vol_gmv], [r_gmv], color='midnightblue', marker='o', markersize= 5)
        ax.text( vol_gmv + 0.005, r_gmv + 0.01, 'GMV', color='midnightblue', fontweight='bold' )
